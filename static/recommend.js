// Replace 'YOUR_API_KEY' below with your API key retrieved from https://www.themoviedb.org // global string to be consistent with future usages elsewhere
const myAPI = "67b00bd018ad5e560a8c2e981d2dd8b3";
$(function () {
...
function load_details(my_api_key, search, isQuerySearch) {
    if (isQuerySearch) {
        url = `https://autocompletion-k57nonapdq-df.a.run.app?task=search&query=${search}`;
    } else {
        url = `https://autocompletion-k57nonapdq-df.a.run.app?task=movie&query=${search}`;
    }
    $.ajax({
        type: 'GET',
        url: url,
...
function get_movie_details(movie_id, my_api_key, movie_title, movie_title_org) {
    $.ajax({
        type: 'GET',
        url: `https://autocompletion-k57nonapdq-df.a.run.app?task=movie&query=${movie_id}`,
